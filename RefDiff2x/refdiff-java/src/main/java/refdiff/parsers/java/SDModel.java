...
import refdiff.core.cst.Visibility;
...
public class SDModel {
	...
	public CstNode createInnerType(String typeName, HasChildrenNodes parent, String sourceFilePath, CharSequence fileContent, AbstractTypeDeclaration ast, String nodeType) {
		...
		cstNode.setOracleStyleName(n.getOracleStyleName()+"."+typeName);
	...
	public CstNode createType(String typeName, String packageName, HasChildrenNodes parent, String sourceFilePath, CharSequence fileContent, AbstractTypeDeclaration ast, String nodeType) {
		...
		cstNode.setSimpleName(typeName);
		cstNode.setNamespace(namespace);
		cstNode.setOracleStyleName(namespace+typeName);
		parent.addNode(cstNode);
		ITypeBinding binding = ast.resolveBinding();
		if (binding != null) {
			keyMap.put(binding.getKey(), cstNode);
	...
	public CstNode createMethod(String methodSignature, HasChildrenNodes parent, String sourceFilePath, CharSequence fileContent, boolean constructor, MethodDeclaration ast, String methodSignOracleStyle) {
		...
		cstNode.setOracleStyleName(methodSignOracleStyle);
		cstNode.setVisibility(Visibility.getVisibility(ast.getModifiers()));
		cstNode.setAbstractMethod(ast.getBody()==null || ast.getBody().statements().size() == 0);
		parent.addNode(cstNode);
