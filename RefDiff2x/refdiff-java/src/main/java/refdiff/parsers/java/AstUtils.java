...
import static refdiff.parsers.java.Visibility.getVisibility;
...
public class AstUtils {
	...
	public static String getOracleStyleSignatureFromMethodDeclaration(MethodDeclaration methodDeclaration) {
		...
		StringBuilder sb = new StringBuilder();
		sb.append(getVisibility(methodDeclaration.getModifiers())).append(" ");
		sb.append(methodName);
		sb.append('(');
		@SuppressWarnings("unchecked")
		Iterator<SingleVariableDeclaration> parameters = methodDeclaration.parameters().iterator();
		while (parameters.hasNext()) {
			...
			Type parameterType = parameter.getType();
			String typeName = normalizeTypeNameOracleStyle(parameterType, parameter.getExtraDimensions(), parameter.isVarargs());
		...
		if(!methodDeclaration.isConstructor()) {
			...
			if (retType == null) {
				...
				sb.append(normalizeTypeNameOracleStyle(retType, 0, false));
			}
		}
	...
	public static String normalizeTypeNameOracleStyle(Type type, int extraDimensions, boolean varargs) {
		StringBuilder sb = new StringBuilder();
		// String rawTypeName = stripQualifiedTypeName(stripTypeParamsFromTypeName(type.toString()));
