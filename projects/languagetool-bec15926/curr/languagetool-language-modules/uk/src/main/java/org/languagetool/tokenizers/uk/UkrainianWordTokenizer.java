...
//  private static final char DECIMAL_SPACE_SUBST = '\uE008';
  ...
  // braces in words
  private static final Pattern BRACE_IN_WORD_PATTERN = Pattern.compile("([а-�?іїєґ'])\\(([а-�?іїєґ']+)\\)", Pattern.CASE_INSENSITIVE|Pattern.UNICODE_CASE);
  private static final char LEFT_BRACE_SUBST = '\uE005';
  private static final char RIGHT_BRACE_SUBST = '\uE006';
  // abbreviation dot
  //TODO: л.�?., ч.л./ч. л., �?т. л., р. х.
  private static final Pattern ABBR_DOT_PATTERN = Pattern.compile("(ти�?)\\.([ \u00A0]+[а-�?іїєґ])");
  private static final Pattern ABBR_DOT_PATTERN1 = Pattern.compile("([^а-�?іїєґ�?-ЯІЇЄ�?'-]лат)\\.([ \u00A0]+[a-zA-Z])");
  private static final Pattern ABBR_DOT_PATTERN2 = Pattern.compile("([�?а]кад|[Пп]роф|[Дд]оц|[�?а]�?и�?т|вул|о|р|ім)\\.([\\s\u00A0]+[�?-ЯІЇЄ�?])");
  // �?ело, мі�?то, річка (�?кщо з цифрою: �?екунди, метри, роки) - з роками �?кладно
  private static final Pattern ABBR_DOT_PATTERN5 = Pattern.compile("((?:[0-9]|кв\\.?|куб\\.?)[\\s\u00A0]+[�?м])\\.");
  private static final Pattern ABBR_DOT_PATTERN3 = Pattern.compile("(�?)\\.(-г)\\.");
  private static final Pattern ABBR_DOT_PATTERN4 = Pattern.compile("([^а-�?іїєґ'-][векнпр�?тцч]{1,2})\\.([екмнпр�?тч]{1,2})\\.");
//  private static final Pattern ABBR_DOT_PATTERN5 = Pattern.compile("(�?т)\\.([ \u00A0]*[0-9]+)");
  private static final Pattern ABBR_DOT_PATTERN6 = Pattern.compile("([^а-�?іїєґ�?-ЯІЇЄ�?'-]((та|й) ін|амер|англ|бл(изьк)?|вірм|грец(ьк)|див|дол|до�?л|доц|е|ел|жін|за�?т|зв|ім|івр|і�?п|італ|к|кв|[1-9]-кімн|кімн|кл|коп|м|н|напр|обл|п|пен|перекл|пл|пор|поч|прибл|пров|про�?п|р|[Рр]ед|[Рр]еж|рр|рт|руб|�?|[С�?]в|�?оц|�?півавт|�?т|�?тол|�?тор|табл|тел|укр|філол|фр|франц|ч|чайн|чол|ц|шт))\\.");
