...
//    assertEmptyMatch("�?к у Конан Дойла")).length);
    ...
    matches = rule.match(langTool.getAnalyzedSentence("зацікавлених у ви кори�?танні"));
    assertEquals(1, matches.length);
...
//    assertEquals(1, matches.length);
    ...
    matches = rule.match(langTool.getAnalyzedSentence("— О пан Єзу�?, захи�?ти їх!"));
    assertEquals(1, matches.length);
    
    matches = rule.match(langTool.getAnalyzedSentence("�?а фото: З Голлівуду Яринка Шу�?т привезла дві золоті медалі"));
    assertEquals(1, matches.length);
  }
