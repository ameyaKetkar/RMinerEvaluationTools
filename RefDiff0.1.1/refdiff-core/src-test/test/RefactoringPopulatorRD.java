...
//		ChangeAttributeType(new BigInteger("137438953472")),
	...
	public static List<Root> getFSERefactorings(BigInteger flag, String outputPath) throws JsonParseException, JsonMappingException, IOException {
...
//				.filter(x->x.sha1.equals("711992f22ae6d6dfc43bdb4c01bf8f921d175b38"))
			//	.filter(x -> !commits.contains(x.sha1))
//				.filter(x->x.url.contains("drill") )
				.collect(Collectors.toList());

		List<Map.Entry<String, List<Root>>> m = roots.stream().collect(groupingBy(x -> x.repository))
				.entrySet().stream().sorted(Comparator.comparing(Map.Entry::getKey))
				.collect(toList());
	//	Collections.reverse(m);

	//	roots = m.stream().flatMap(x->x.getValue().stream()).collect(toList());
		//roots = m.subList(0,m.size()/2).stream().flatMap(x->x.getValue().stream()).collect(toList());
		
		//List<Integer> l = Arrays.asList(1155451, 1151906, 1140956, 1120125, 1145105, 1152344, 1153579, 1106365, 1127907,1158913,1143517,1107905,1124355);
		//List<Integer> l = Arrays.asList(1149802, 1122167, 1122164, 1116686, 1156317, 1158180, 1152936, 1141428, 1123684,1156276,1143196,1139791,1149799);
		//roots = roots.stream().filter(x->l.contains(x.id)).collect(toList());
//		roots  = m.subList(m.size() / 2, m.size()).stream().flatMap(x -> x.getValue().stream()).collect(toList());
